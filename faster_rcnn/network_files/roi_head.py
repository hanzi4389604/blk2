...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
          #  print("proposals_in_image",len(proposals_in_image))
            #print("gt_boxes_in_image",gt_boxes_in_image)
            if gt_boxes_in_image.numel() == 0:  # 该张图像中没有gt框，为背景
                print("gt_boxes_in_image.numel()  ====== 0")
                # background image
                ...
                #print("gt_boxes_in_image",len(gt_boxes_in_image))
                #print("match_quality_matrix",match_quality_matrix[0].shape)

                # 计算proposal与每个gt_box匹配的iou最大值，并记录索引，
                # iou < low_threshold索引值为 -1， low_threshold <= iou < high_threshold索引值为 -2
                matched_idxs_in_image = self.proposal_matcher(match_quality_matrix)
               # print("matched_idxs_in_image",matched_idxs_in_image)
                # 限制最小值，防止匹配标签时出现越界的情况
                # 注意-1, -2对应的gt索引会调整到0,获取的标签类别为第0个gt的类别（实际上并不是）,后续会进一步处理
                clamped_matched_idxs_in_image = matched_idxs_in_image.clamp(min=0)
            #    print("clamped_matched_idxs_in_image",clamped_matched_idxs_in_image)
             #   print("gt_labels_in_image",gt_labels_in_image)
                # 获取proposal匹配到的gt对应标签
                labels_in_image = gt_labels_in_image[clamped_matched_idxs_in_image]
              #  print("labels_in_image",labels_in_image)
                ...
                between = torch.where(torch.eq(labels_in_image, -1))[0]
             #   print("ignore",between.shape)
    ...
    def select_training_samples(self,
       ...
       # print("proposals",proposals[0][0])
        device = proposals[0].device

        # 获取标注好的boxes以及labels信息
        gt_boxes = [t["boxes"].to(dtype) for t in targets]
        #print("len gt_boxes",len(gt_boxes[0]))
        gt_labels = [t["labels"] for t in targets]

        # append ground-truth bboxes to proposal
        # 将gt_boxes拼接到proposal后面
        proposals = self.add_gt_proposals(proposals, gt_boxes)
       # print("len proposals",len(proposals[0]))
        # get matching gt indices for each proposal
        # 为每个proposal匹配对应的gt_box，并划分到正负样本中
        matched_idxs, labels = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels)
       # print("len matched_idxs  labels",len(matched_idxs[0]), len(labels[0]))
        # sample a fixed proportion of positive-negative proposals
        # 按给定数量和比例采样正负样本
        sampled_inds = self.subsample(labels)
       # print("sampled_inds",len(sampled_inds[0]))
        matched_gt_boxes = []
       ...
       # print("num_images",num_images)
        ...
        for img_id in range(num_images):
           # print("proposals",len(proposals[img_id]))
            # 获取每张图像的正负样本索引
           ...
           # print("proposals[img_id]",proposals[img_id].shape)
            ...
            # 获取对应正负样本的gt索引信息
            matched_idxs[img_id] = matched_idxs[img_id][img_sampled_inds] #must equal to 0,1,2,3,4, the number of gt
            #print("matched_idxs",matched_idxs[img_id])
            gt_boxes_in_image = gt_boxes[img_id]
            #print("gt_boxes_in_image",gt_boxes_in_image.shape)
            if gt_boxes_in_image.numel() == 0:
                gt_boxes_in_image = torch.zeros((1, 4), dtype=dtype, device=device)
            # 获取对应正负样本的gt box信息
           # print("gt_boxes_in_image[matched_idxs[img_id]]",gt_boxes_in_image[matched_idxs[img_id]])
           # print("gt_boxes_in_image[matched_idxs[img_id]] shape",gt_boxes_in_image[matched_idxs[img_id]].shape)
            matched_gt_boxes.append(gt_boxes_in_image[matched_idxs[img_id]])

        # 根据gt和proposal计算边框回归参数（针对gt的）
        regression_targets = self.box_coder.encode(matched_gt_boxes, proposals)
       # print("matched_gt_boxes, proposals",len(matched_gt_boxes[0]), len(proposals[0]),proposals[0].shape)
    ...
    def forward(self,
        ...
        Arguments:
            ...
            targets (List[Dict])
        """
    #    for keys, values in enumerate(features.keys()):
    #        print(keys, features[values].shape)
        #print("proposals[values].shape",proposals[0].shape)
        # 检查targets的数据类型是否正确
        ...
        if self.training:
            # 划分正负样本，统计对应gt的标签以及边界框回归信息
            #print("targets",targets[0]["boxes"].shape)
      ...
      #  print("proposals",len(proposals),proposals[0].shape)
