...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
     ...
     #   print('aspect_ratios is', aspect_ratios)
        h_ratios = torch.sqrt(aspect_ratios)
        w_ratios = 1.0 / h_ratios
       # print('h_rations is', h_ratios)
        #print('w_ratios is', w_ratios)
        # [r1, r2, r3]' * [s1, s2, s3]
   ...
   #         print("size is ",sizes, "aspect ratio is", aspect_ratios) 
        cell_anchors = [
    ...
    def grid_anchors(self, grid_sizes, strides):
     ...
     #       print('size is', size)
      #      print('stride is', stride)
       #     print('base_anchors is', base_anchors)
          ...
          #  print("grid_width is",grid_width)
    #        print("shifts_x is",len(shifts_x),shifts_x)
          #  print("stride_width is",stride_width)
            # shape: [grid_height] 对应原图上的y坐标(行)
            shifts_y = torch.arange(0, grid_height, dtype=torch.float32, device=device) * stride_height
      #      print("shifts_y is",len(shifts_y),shifts_y)

            # 计算预测特征矩阵上每个点对应原图上的坐标(anchors模板的坐标偏移量)
            # torch.meshgrid函数分别传入行坐标和列坐标，生成网格行坐标矩阵和网格列坐标矩阵
            # shape: [grid_height, grid_width]
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
      #      print("shifts_x1 is",len(shift_x),shift_x.shape)
       #     print("shift_y1 is",len(shift_y),shift_y.shape)
        #    print("shifts_x1 is",len(shift_x),shift_x.shape,shift_x[:500])
         #   print("shift_y1 is",len(shift_y),shift_y.shape,shift_y[:500])
     ...
     #       print("shifts shape is",shifts.shape)
            # For every (base anchor, output anchor) pair,
            # offset each zero-centered base anchor by the center of the output anchor.
            # 将anchors模板与原图上的坐标偏移量相加得到原图上所有anchors的坐标信息(shape不同时会使用广播机制)
            shifts_anchor = shifts.view(-1, 1, 4) + base_anchors.view(1, -1, 4)
    #        print("shifts anchor is",shifts_anchor.shape)
            anchors.append(shifts_anchor.reshape(-1, 4))
       #     for ss in range (len(anchors)):
      #          print(len(anchors))
       #         print("anchor shape is ",anchors[ss].shape)
           # print("done")
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
       ...
       # print("cache key AAAAAAA is", key)
        # self._cache是字典类型
    ...
    def forward(self, image_list, feature_maps):
        ...
        # print("image_list",image_list) #image_size torch.Size([800, 1216])
       # for feature_map in feature_maps:
       #     print("feature_map.shape[:]",feature_map.shape) #feature_map.shape[:] torch.Size([3, 256, 304, 200])
        grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])
#        print("grid_sizes",grid_sizes)#grid_sizes [torch.Size([304, 200]), torch.Size([152, 100]), torch.Size([76, 50]), torch.Size([38, 25]), torch.Size([19, 13])]
        # 获取输入图像的height和width
        image_size = image_list.tensors.shape[-2:]
        
     #   print("image_size",image_list.tensors.shape)
        # 获取变量类型和设备类型
       ...
       # print("strides original is",strides)
        # 根据提供的sizes和aspect_ratios生成anchors模板
       # print("start ")
 ...
 #       for sss in range(len(anchors_over_all_feature_maps)):
 #           print("anchors_over_all_feature_maps sss", anchors_over_all_feature_maps[sss].shape)
        
        anchors = torch.jit.annotate(List[List[torch.Tensor]], [])
        # 遍历一个batch中的每张图像
     #   print("image_list.image_sizes",image_list.image_sizes)
    ...
    #    for sss in range(len(anchors)):
    #        print("lenth of anchors", len(anchors))
    #        print("SSS is",sss)
     #       print("anchors_over_all_feature_maps ssssssss", anchors[sss][0].shape)
        # Clear the cache in case that memory leaks.
        # 将每一张图像的所有预测特征层的anchors坐标信息拼接在一起
        # anchors是个list，每个元素为一张图像的所有anchors信息
        anchors = [torch.cat(anchors_per_image) for anchors_per_image in anchors]
       # print("anchors type ",anchors[0].shape)
...
class RPNHead(nn.Module):
    ...
    #x is 5 layers of feature maps head input x torch.Size([3, 256, 200, 304])
#head input x torch.Size([3, 256, 100, 152])
#head input x torch.Size([3, 256, 50, 76])
#head input x torch.Size([3, 256, 25, 38])
#ead input x torch.Size([3, 256, 13, 19])
#        for xxx in range(len(x)):
#            print("head input x", x[xxx].shape)
        ...
        #for xx in range(len(bbox_reg)):
        #    print("bbox_reg",bbox_reg[xx].shape)
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
       ...
       # print("AxC is",box_cls_per_level.shape)
        # # [batch_size, anchors_num_per_position * 4, height, width]
        Ax4 = box_regression_per_level.shape[1]
      #  print("Ax4 is", box_regression_per_level.shape)
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        """
        labels = []
      #  print("self._post_nms_top_n",self._post_nms_top_n)
        
        matched_gt_boxes = []
        # 遍历每张图像的anchors和targets
        #print("lenth of anchor and targets",len(anchors), len(targets))
    #    print("len of targets",len(targets))
        for anchors_per_image, targets_per_image in zip(anchors, targets):
          #  print("anchors_per_image, targets_per_image",anchors_per_image.shape, targets_per_image)
            gt_boxes = targets_per_image["boxes"]
         #   print(gt_boxes.numel())
            if gt_boxes.numel() == 0:
               ...
               # print("gt_boxes, anchors_per_image",anchors_per_image.shape, gt_boxes.shape)
                #print("match_quality_matrix, ",match_quality_matrix[0][:100])
                # 计算每个anchors与gt匹配iou最大的索引（如果iou<0.3索引置为-1，0.3<iou<0.7索引为-2）
                matched_idxs = self.proposal_matcher(match_quality_matrix)
               # xx= matched_idxs==7
               # print("matched_idxs, ",xx)
            ...
            #    print("labels_per_image   matched_gt_boxes_per_image",labels_per_image[:500], matched_gt_boxes_per_image.shape)
    ...
    #    for xx in range(len(num_anchors_per_level)):
    #        print("num_anchors_per_level ness ",num_anchors_per_level[xx])
        offset = 0
        # 遍历每个预测特征层上的预测目标概率信息
      ...
      #  print("objectness",objectness.shape)
       ...
       # print("objectness",objectness.shape)
        ...
        levels = [torch.full((n, ), idx, dtype=torch.int64, device=device)
                  for idx, n in enumerate(num_anchors_per_level)]
      #  for ss in range(len(levels)):
      #      print("levels  111",levels[ss][:100])
        levels = torch.cat(levels, 0)
      #  print("levels 22",levels.shape)

        # Expand this tensor to the same size as objectness
        levels = levels.reshape(1, -1).expand_as(objectness)
      #  print("levels 33",levels.shape)

        # select top_n boxes independently per level before applying nms
        # 获取每张预测特征图上预测概率排前pre_nms_top_n的anchors索引值
        top_n_idx = self._get_top_n_idx(objectness, num_anchors_per_level)
       # print("top_n_idx 33",top_n_idx[0][:100])
        image_range = torch.arange(num_images, device=device)
        
        batch_idx = image_range[:, None]  # [batch_size, 1]
       # print("batch_idx 33",batch_idx)
        # 根据每个预测特征层预测概率排前pre_nms_top_n的anchors索引值获取相应概率信息
       # print("objectness 33",objectness.shape)
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        print("sampled_inds",len(sampled_inds))
     ...
     #   print("labels[sampled_inds",len(labels))
       # print("labels[sampled_inds",len(labels[sampled_inds]))
    ...
    def forward(self,
       ...
       #     print("features list is",features[xx].shape)
        #print("features keys is",features.keys())
      ...
      #  Objectiveness shape is 5 layers * 3 imagers * 3shapes* (dots need to be calculated)*width*height
  #      for ss in pred_bbox_deltas:
  #          print(len(pred_bbox_deltas))
  #          print(len(ss))
  #          print("objectness",ss[1].shape, ss[2].shape)
            
        # 生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        anchors = self.anchor_generator(images, features)
       # anxhors shape is 3 imagers * (5 layers * 3shapes* width*height) *4
        #for ss in anchors:
        #    print(len(ss))
        #    print(len(anchors))
  #          print(ss.shape)
         #   print("objectness",ss[1].shape, ss[4].shape)
        # batch_size
        num_images = len(anchors)
        #print("num_images", num_images)     equal to batch_size
        ...
        # 调整内部tensor格式以及shape, 批次和3个anchor/pixel 全部合并到一起
        objectness, pred_bbox_deltas = concat_box_prediction_layers(objectness,
                                                                    pred_bbox_deltas)

       # print("objectness",objectness.shape,pred_bbox_deltas.shape)
       ...
       # print("image keys",images.image_sizes)
       ...
       # print("boxes", boxes[0].shape)
        ...
        if self.training:
         ...
         #   print("regression_targets",regression_targets[0].shape)
